...
const ClientClaims = () => {
	...
	const save = () => {
		...
		if (configuration.secondaryClaims && blockState.some((b) => claims[b.id].secondary === null)) {
	...
	const updateClaims = (blockId: Block["id"], newClaims: BlockClaims[number]) => {
		if (
			!configuration.allowMultipleClaimsOfSameArchetype &&
			blockState.some((b) => {
				if (b.id === blockId) return false;

				if (newClaims.primary !== null && claims[b.id].primary === newClaims.primary) {
					toast.warning("Nelze vybrat 2 stejné primární přednášky");

					return true;
				}

				if (newClaims.secondary !== null && newClaims.secondary === claims[b.id].primary) {
					toast.warning("Nelze zvolit stejnou sekundární přednášku jako primární v jiném bloku");

					return true;
				}

				if (newClaims.secondary !== null && newClaims.secondary === claims[b.id].secondary) {
		...
		<div className="flex flex-col gap-4">
			{configuration.closeClaimsOn.getTime() < Date.now() ? (
				<Alert>
					...
					<AlertDescription className="text-muted-foreground">
						Pokud máte nějaký problém s vašimi přednáškami, kontaktujte prosím organizátora akce.
					</AlertDescription>
				</Alert>
			) : (
				<Alert>
					<AlertDescription className="text-muted-foreground">{configuration.claimsNote}</AlertDescription>
				</Alert>
			)}
			{!loadingBlockState ? (
				<>
					{blockState.map((block) => (
						<BlockElement
							...
							onClaimsChange={(newClaims) => updateClaims(block.id, newClaims)}
			...
			<div className="flex flex-wrap items-center justify-end gap-4">
				...
				{canEditClaimsNow(user) ? (
					...
					<Button variant="outline">Volba přednášek je uzavřena</Button>
				)}
			</div>
		</div>
	);
