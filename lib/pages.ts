...
export type PageInfo = {
	...
	object,
	(Either<{ showInSidebar: true }, { showOnHomepage: true }> | { showInSidebar: true; showOnHomepage: true }) & {
		name: string;
		icon: LucideIcon;
...
export const getPages = ({ isAttending, isTeacher, isAdmin }: UserPermissions): PageInfo[] => {
	...
	if (isAttending)
		pages.push(
			...(configuration.closeClaimsOn.getTime() <= Date.now()
				? [
						{
							name: configuration.showEntireSchedule ? "Můj harmonogram" : "Kde mám teď být?",
							path: "/attending",
							file: "/attending/now",
							showInSidebar: true,
							showOnHomepage: true,
