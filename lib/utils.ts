...
export const asyncInlineCatch = async <T>(fn: () => Promise<T>): Promise<[T, null] | [null, Error]> => {
	try {
...
export const UserError = (message: string | Error | ZodError): UserErrorType => ({
	type: "error",
	message:
		message instanceof ZodError ? message.errors[0].message : message instanceof Error ? message.message : message,
});

export const UnauthorizedError = (): UserErrorType => UserError("K provedení této akce nemáte dostatečná oprávnění");

export const catchUserError = <T>(data: UserErrorType | T): [T, null] | [null, UserErrorType] =>
	(data as UserErrorType | { type?: unknown })?.type === "error" ? [null, data as UserErrorType] : [data as T, null];

export const pluralHelper = (count: number, singular: string, TwoToFile: string, Many: string | null = null) =>
	count === 1 ? singular : (count >= 2 && count <= 4) || !Many ? TwoToFile : Many;
...
export function createMapById<T, K extends keyof T>(data: T[], idKey: K): Map<T[K], T>;
...
export function createMapById<T, K extends keyof T>(data: T[], idKey: K, dataKey?: keyof T) {
