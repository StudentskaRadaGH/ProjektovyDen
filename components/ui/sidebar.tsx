...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, onDrag, onAnimationEnd, ...props }, ref) => {
		...
		return (
			<Drawer
				...
				<DrawerContent
					...
					<div className="flex h-full w-full flex-col p-2 text-base">{children}</div>
				</DrawerContent>
			</Drawer>
		);
	}
...
const SidebarTrigger = React.forwardRef<React.ElementRef<typeof Button>, React.ComponentProps<typeof Button>>(({ className, onClick, ...props }, ref) => {
	...
	return (
		<Button
			...
			{...props}>
			<ChevronsUp className="size-full" />
