...
export const archetypes = pgTable("archetypes", {
	...
	detailedDescription: varchar("detailed_description", { length: 2047 }),
...
export const events = pgTable(
	...
	(table) => [index("event_block_index").on(table.block), index("event_place_index").on(table.place), index("event_archetype_index").on(table.archetype), index("event_block_archetype_index").on(table.block, table.archetype)]
);
...
export const presenters = pgTable(
	...
	(table) => [index("presenter_user_index").on(table.user), index("presenter_event_index").on(table.event)]
);
...
export const interests = pgTable(
	...
	(table) => [index("interest_user_index").on(table.user), index("interest_archetype_index").on(table.archetype)]
);
...
export const attendance = pgTable(
	...
	(table) => [index("attendance_user_index").on(table.user), index("attendance_event_index").on(table.event)]
);
...
export const blockArchetypeLookup = pgTable(
	...
	(table) => [uniqueIndex("lookup_block_archetype_index").on(table.block, table.archetype), index("lookup_block_index").on(table.block), index("lookup_archetype_index").on(table.archetype)]
);
