...
import { and, archetypes, blockArchetypeLookup, claims, db, eq, interests } from "@/db";
...
export const addArchetype = async (unsafe: addArchetypeSchema) => {
	...
	const newId = (await db.insert(archetypes).values(archetype).returning({ insertedId: archetypes.id }))[0].insertedId;
...
export const editArchetype = async (unsafe: editArchetypeSchema) => {
	...
	await db
		...
		.set({
			...
			detailedDescription: archetype.detailedDescription,
		})
		.where(eq(archetypes.id, archetype.id));
...
export const deleteArchetype = async (id: number) => {
	...
	if (exists.events.length > 0) return UserError("Nelze smazat přednášku, která je přiřazena k události");
		...
		archetypeClaims
			...
			.map((claim) =>
				db
					.update(claims)
					.set({ secondary: false })
					.where(and(eq(claims.user, claim.user), eq(claims.block, claim.block)))
			)
	);

	await db.delete(blockArchetypeLookup).where(eq(blockArchetypeLookup.archetype, id));
