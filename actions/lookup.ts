...
import { archetypes as archetypesTable, asc, blockArchetypeLookup, blocks as blocksTable, claims as claimsTable, db, eq, events as eventsTable } from "@/db";
...
export const recalculateArchetypeInterested = async () => {
	...
	await Promise.allSettled(archetypes.map((archetype) => db.update(archetypesTable).set({ interested: archetype.interests.length }).where(eq(archetypesTable.id, archetype.id))));
};
...
export const recalculateEventAttending = async () => {
	...
	await Promise.allSettled(events.map((event) => db.update(eventsTable).set({ attending: event.attendances.length }).where(eq(eventsTable.id, event.id))));
};
	...
	await db.insert(blockArchetypeLookup).values(
		blocks.flatMap((block) =>
			archetypes.map((archetype) => {
				const events = archetype.events.filter((e) => e.block === block.id);
				const capacity = events.reduce((acc, e) => acc + e.capacity, 0);
				let numOfClaims = 0;

				claims.forEach((c) => {
					if (c.block === block.id && c.archetype === archetype.id) numOfClaims++;
				});
